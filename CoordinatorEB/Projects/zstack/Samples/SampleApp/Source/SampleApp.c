...
void SampleApp_SendPointToPointMessage( mtOSALSerialData_t *cmdMsg );
...
void SampleApp_Init( uint8 task_id )
{
  ...
  // 网蜂点对点通讯定义
    ...
    Point_To_Point_DstAddr.addr.shortAddr = 0x8779;  //终端地址
      //发给终端
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( MSGpkt )
    {
      ...
      {
        case CMD_SERIAL_MSG:
          SampleApp_SendPointToPointMessage((mtOSALSerialData_t *)MSGpkt);
          break;
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  switch ( pkt->clusterId)
  {
    case SAMPLEAPP_POINT_TO_POINT_CLUSTERID:
     ...
     HalUARTWrite(0,"ShortAddr:0x",12);
